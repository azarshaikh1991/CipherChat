...
pipeline {
...
} // <--- Yeh raha node block ka aakhri closing brace. Iske baad kuch nahi hona chahiye!node {
    def DOCKER_HUB_CREDS = 'docker-hub-credentials'
    def DOCKER_HUB_USER  = 'azarr1991'
    def IMAGE_NAME       = 'arzo-messenger'
    def IMAGE_TAG        = "${env.BUILD_NUMBER}"
    ...
    try {
        ...
        stage('Install Dependencies') {
            // NVM exact path include kiya gaya hai
            sh '''
                export PATH="/Users/azarshaikh/.nvm/versions/node/v24.19.0/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH"
                npm install
            '''
        }
